...
class ExactSIMClassifier(nn.Module):
    ...
    Args:
        ...
        num_classes (int): 1 for binary (returns probabilities of shape (batch,)).
            ...
            returns logits of shape (batch, c). Also accepted as `n_classes`.
        ...
        noise_model: optional PennyLane noise model, applied via qml.add_noise.
        shots (int): finite shot budget for the device; None means analytic.
        **device_kwargs: forwarded to qml.device.
    """

    def __init__(self, n_qubits, n_layers=3, device_name='default.qubit',
                 pauli_strings=None, num_classes=None, n_classes=None,
                 normalize_alpha=False, noise_model=None, shots=None,
                 **device_kwargs):
        super().__init__()
        # `n_classes` and `num_classes` are accepted interchangeably.
        if num_classes is None:
            num_classes = 1 if n_classes is None else n_classes
        ...
        self.w = nn.Parameter(
            (torch.randn(num_classes, self.n_paulis) * 0.01).double())
